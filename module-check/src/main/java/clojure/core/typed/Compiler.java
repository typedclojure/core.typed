...
final static Keyword CONST_KEY = Keyword.intern(null, "const");
  // end copied from RT.java

// Copied from LispReader.java
...
// end Copied from LispReader.java
...
static final Var WARN_ON_REFLECTION = (Var)Clojure.var("clojure.core/*warn-on-reflection*");
static final Var PRINT_DUP = (Var)Clojure.var("clojure.core/*print-dup*");
static final Var IN_NS_VAR = (Var)Clojure.var("clojure.core/in-ns");
static final Var NS_VAR = (Var)Clojure.var("clojure.core/ns");
static final Var ALLOW_UNRESOLVED_VARS = (Var)Clojure.var("clojure.core/*allow-unresolved-vars*");
static final Var FN_LOADER_VAR = (Var)Clojure.var("clojure.core/*fn-loader*");

public static final Var MACROEXPAND_ONE = Var.create(Clojure.var("clojure.core/macroexpand-1")).setDynamic();

// Copied from Reflector.java
...
static Object invokeMatchingMethod(String methodName, List methods, Object target, Object[] args)
  ...
  if(methods.isEmpty())
    ...
    {
    m = (java.lang.reflect.Method) methods.get(0);
    boxedArgs = boxArgs(m.getParameterTypes(), args);
    }
  else //overloaded w/same arity
    {
    java.lang.reflect.Method foundm = null;
    for(Iterator i = methods.iterator(); i.hasNext();)
      {
      m = (java.lang.reflect.Method) i.next();
  ...
  if(!Modifier.isPublic(m.getDeclaringClass().getModifiers()))
    ...
    java.lang.reflect.Method oldm = m;
    m = clojure.lang.Reflector.getAsMethodOfPublicBase(target.getClass(), m);
    if(m == null)
  ...
  try
    {
    return clojure.lang.Reflector.prepRet(m.getReturnType(), m.invoke(target, boxedArgs));
    }
  catch(Exception e)
    {
...
static boolean isCongruent(Class[] params, Object[] args){
  ...
  if(params.length == args.length)
  {
    ...
    for(int i = 0; ret && i < params.length; i++)
    {
      ...
      Class paramType = params[i];
      ret = clojure.lang.Reflector.paramArgTypeMatch(paramType, argType);
    }
  }
  return ret;
}
//End copy from Reflector.java

// Start copy from Intrinsics.java
private static Object[] oa(Object... arr){
  return arr;
}
//map to instructions terminated with comparator for branch to false
static IPersistentMap intrinsics_preds = RT.map(
...
static IPersistentMap intrinsics_ops = RT.map(
...
final static Type TUPLE_TYPE = Type.getType(clojure.core.typed.temp.Tuple.class);
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object frm) {
			...
			if(maybeField)    //field
				...
				if(c != null) {
					return new StaticFieldExpr(line, column, c, munge(sym.getName()), tag);
				} else
					return new InstanceFieldExpr(line, column, instance, munge(sym.getName()), tag, (((Symbol)RT.third(form)).getName().charAt(0) == '-'));
				}
			else
				{
				...
				if(c != null)
					return new StaticMethodExpr(source, line, column, tag, c, munge(sym.getName()), args);
				else
					return new InstanceMethodExpr(source, line, column, tag, instance, munge(sym.getName()), args);
				}
		}
	}
	...
	public Object eval() {
		try
			...
			if(method != null)
				...
				ms.add(method);
				return invokeMatchingMethod(methodName, ms, targetval, argvals);
				}
...
static class StaticMethodExpr extends MethodExpr{
	...
	public Object eval() {
		try
			...
			if(method != null)
				...
				LinkedList ms = new LinkedList();
				ms.add(method);
				return invokeMatchingMethod(methodName, ms, null, argvals);
				}
	...
	public boolean canEmitIntrinsicPredicate(){
		return method != null && RT.get(intrinsics_preds, method.toString()) != null;
	}
	...
	public void emitIntrinsicPredicate(C context, ObjExpr objx, GeneratorAdapter gen, Label falseLabel){
		...
		if(method != null)
			...
			Object[] predOps = (Object[]) RT.get(intrinsics_preds, method.toString());
	...
	public void emitUnboxed(C context, ObjExpr objx, GeneratorAdapter gen){
		if(method != null)
			...
			Object ops = RT.get(intrinsics_ops, method.toString());
			if(ops != null)
				{
...
//		loader.registerQuotedVal(id, v);
	...
	public ConstantExpr(Object v, boolean isQuote){
		this.v = v;
		this.id = registerConstant(v);
	...
	static class Parser implements IParser{
		...
		public Expr parse(C context, Object form){
			...
			if(v instanceof Number)
				...
				return new ConstantExpr(v, true);
		}
	}
}
...
public static class NewExpr implements Expr{
	...
	public Object eval() {
		...
		if(this.ctor != null)
			...
			try
				{
				return ctor.newInstance(boxArgs(ctor.getParameterTypes(), argvals));
				}
			catch(Exception e)
				{
				throw Util.sneakyThrow(e);
...
public static class EmptyExpr implements Expr{
	...
	final static Type EMPTY_LIST_TYPE = Type.getType((Class)Clojure.var("clojure.core/class").invoke(PersistentList.EMPTY));
...
public static class VectorExpr implements Expr{
	...
	public void emit(C context, ObjExpr objx, GeneratorAdapter gen){
        if(args.count() <= clojure.core.typed.temp.Tuple.MAX_SIZE)
            {
            for(int i = 0; i < args.count(); i++) {
      			((Expr) args.nth(i)).emit(C.EXPRESSION, objx, gen);
      			}
...
public static Object macroexpand1(Object x) {
  return MACROEXPAND_ONE.invoke(x);
}
		...
		if(namespaceFor(sym) == null)
			...
			if(c != null)
				...
				if(Reflector.getField(c, sym.getName(), true) != null)
					return new StaticFieldExpr(lineDeref(), columnDeref(), c, sym.getName(), tag);
...
public static Object load(Reader rdr, String sourcePath, String sourceName) {
	...
	try
		...
		{
    try {
      java.lang.reflect.Field line_field = e.getClass().getDeclaredField("line");
      java.lang.reflect.Field column_field = e.getClass().getDeclaredField("column");
      line_field.setAccessible(true);
      column_field.setAccessible(true);
      throw new CompilerException(sourcePath, (Integer)line_field.get(e), (Integer)column_field.get(e), e.getCause());
