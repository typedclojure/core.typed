...
package clojure.core.typed.lang;
...
public class Compiler implements Opcodes{
  ...
  static IFn TYPED_MACRO_LOOKUP = (IFn) Clojure.var("clojure.core.typed.analyze-clj", "typed-macro-lookup");
	...
	final static Var NS_VAR = (Var) Clojure.var("clojure.core", "ns");
	final static Var IN_NS_VAR = (Var) Clojure.var("clojure.core", "in-ns");
	final static Var ALLOW_UNRESOLVED_VARS = (Var) Clojure.var("clojure.core", "*allow-unresolved-vars*");
...
//String
static final public Var COMPILE_PATH = (Var) Clojure.var("clojure.core", "*compile-path*");
//boolean
static final public Var COMPILE_FILES = (Var) Clojure.var("clojure.core", "*compile-files*");

static final public Var INSTANCE = (Var) Clojure.var("clojure.core", "instance?");

static final public Var ADD_ANNOTATIONS = (Var) Clojure.var("clojure.core", "add-annotations");
...
public static class DefExpr implements Expr{
  ...
  public final Object form;
	...
	public DefExpr(String source, int line, int column, Var var, Expr init, Expr meta, boolean initProvided, boolean isDynamic, boolean shadowsCoreMapping, Symbol sym, Object form){
		this.source = source;
		this.line = line;
		this.column = column;
		this.var = var;
...
//					                    " refers to:" + v);
			...
			if(isDynamic)
			   v.setDynamic();
            if(!isDynamic && sym.getName().startsWith("*") && sym.getName().endsWith("*") && sym.getName().length() > 2)
                {
            ...
            mm = (IPersistentMap) RT.assoc(mm, LINE_KEY, LINE.get()).assoc(COLUMN_KEY, COLUMN.get()).assoc(FILE_KEY, source_path);
			if (docstring != null)
			  mm = (IPersistentMap) RT.assoc(mm, DOC_KEY, docstring);
...
public static class AssignExpr implements Expr{
	...
	public AssignExpr(AssignableExpr target, Expr val, Object form){
		this.target = target;
		this.val = val;
		this.form = form;
	}
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object frm) {
			...
			return new AssignExpr((AssignableExpr) target, analyze(C.EXPRESSION, RT.third(form)), form);
		}
	}
}
...
public static class TheVarExpr implements Expr{
	...
	public TheVarExpr(Var var, Object form){
		this.var = var;
		this.form = form;
	}
...
public static class ImportExpr implements Expr{
	...
	public ImportExpr(String c, Object form){
		this.c = c;
		this.form = form;
	}
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object form) {
			return new ImportExpr((String) RT.second(form), form);
		}
	}
}
...
//				System.out.println("NOT fnexpr for defn var: " + var + "init: " + init.getClass());
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object frm) {
			...
			if(maybeField && !(((Symbol)RT.third(form)).getName().charAt(0) == '-'))
				{
				Symbol sym = (Symbol) RT.third(form);
				if(c != null)
					maybeField = Reflector.getMethods(c, 0, munge(sym.getName()), true).size() == 0;
				else if(instance != null && instance.hasJavaClass() && instance.getJavaClass() != null)
					maybeField = Reflector.getMethods(instance.getJavaClass(), 0, munge(sym.getName()), false).size() == 0;
				}

			if(maybeField)    //field
				{
				Symbol sym = (((Symbol)RT.third(form)).getName().charAt(0) == '-') ?
					Symbol.intern(((Symbol)RT.third(form)).getName().substring(1))
						:(Symbol) RT.third(form);
				Symbol tag = tagOf(form);
				if(c != null) {
					return new StaticFieldExpr(line, column, c, munge(sym.getName()), tag, frm);
				} else
					return new InstanceFieldExpr(line, column, instance, munge(sym.getName()), tag, (((Symbol)RT.third(form)).getName().charAt(0) == '-'), frm);
				}
			else
				{
				...
				if(c != null)
					return new StaticMethodExpr(source, line, column, tag, c, munge(sym.getName()), args, frm, tailPosition);
				else
					return new InstanceMethodExpr(source, line, column, tag, instance, munge(sym.getName()), args, frm, tailPosition);
				}
		}
	}
 ...
 */
    public static Class maybeSpecialTag(Symbol sym) {
		...
		if (c != null)
			return c;
		else if(sym.getName().equals("objects"))
			c = Object[].class;
		else if(sym.getName().equals("ints"))
			c = int[].class;
		else if(sym.getName().equals("longs"))
			c = long[].class;
		else if(sym.getName().equals("floats"))
			c = float[].class;
		else if(sym.getName().equals("doubles"))
			c = double[].class;
		else if(sym.getName().equals("chars"))
			c = char[].class;
		else if(sym.getName().equals("shorts"))
			c = short[].class;
		else if(sym.getName().equals("bytes"))
			c = byte[].class;
		else if(sym.getName().equals("booleans"))
			c = boolean[].class;
		return c;
    }
...
public static class InstanceFieldExpr extends FieldExpr implements AssignableExpr{
  ...
  Class jc;
	...
	public InstanceFieldExpr(int line, int column, Expr target, String fieldName, Symbol tag, boolean requireField, Object form) {
		...
		this.form = form;
		if(field == null && RT.booleanCast(WARN_ON_REFLECTION.deref()))
			{
			if(targetClass == null)
				{
				RT.errPrintWriter()
...
public static class InstanceMethodExpr extends MethodExpr{
	...
	public InstanceMethodExpr(String source, int line, int column, Symbol tag, Expr target, 
														String methodName, IPersistentVector args, Object form, boolean tailPosition)
			{
		this.source = source;
		this.line = line;
		this.column = column;
		...
		if(target.hasJavaClass() && target.getJavaClass() != null)
			...
			if(methods.isEmpty())
				{
				method = null;
				if(RT.booleanCast(WARN_ON_REFLECTION.deref()))
					{
					RT.errPrintWriter()
				...
				method = m;
				if(method == null && RT.booleanCast(WARN_ON_REFLECTION.deref()))
					{
					RT.errPrintWriter()
...
public static class StaticMethodExpr extends MethodExpr{
	...
	public boolean canEmitIntrinsicPredicate(){
		return method != null && RT.get(clojure.core.typed.lang.Intrinsics.preds, method.toString()) != null;
	}
	...
	public void emitIntrinsicPredicate(C context, ObjExpr objx, GeneratorAdapter gen, Label falseLabel){
		...
		if(method != null)
			...
			Object[] predOps = (Object[]) RT.get(clojure.core.typed.lang.Intrinsics.preds, method.toString());
	...
	public void emitUnboxed(C context, ObjExpr objx, GeneratorAdapter gen){
		if(method != null)
			...
			Object ops = RT.get(clojure.core.typed.lang.Intrinsics.ops, method.toString());
			if(ops != null)
				{
...
public static class MonitorEnterExpr extends UntypedExpr{
	...
	public MonitorEnterExpr(Expr target, Object form){
		this.target = target;
		this.form = form;
	}
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object form) {
			return new MonitorEnterExpr(analyze(C.EXPRESSION, RT.second(form)), form);
		}
	}
}
...
public static class MonitorExitExpr extends UntypedExpr{
	...
	public MonitorExitExpr(Expr target, Object form){
		this.target = target;
		this.form = form;
	}
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object form) {
			return new MonitorExitExpr(analyze(C.EXPRESSION, RT.second(form)), form);
		}
	}
...
public static class TryExpr implements Expr{
	...
	public static class CatchClause{
		...
		public CatchClause(Class c, LocalBinding lb, Expr handler, Object form) {
			this.c = c;
			this.lb = lb;
				...
				if(!Util.equals(op, CATCH) && !Util.equals(op, FINALLY))
					...
					if(Util.equals(op, CATCH))
						...
						try
							...
							catches = catches.cons(new CatchClause(c, lb, handler, f));
							}
						finally
							{
							Var.popThreadBindings();
...
public static class ThrowExpr extends UntypedExpr{
	...
	public ThrowExpr(Expr excExpr, Object form){
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object form) {
			...
			return new ThrowExpr(analyze(C.EXPRESSION, RT.second(form)), form);
		}
	}
}
...
public static class NewExpr implements Expr{
	...
	public NewExpr(Class c, IPersistentVector args, int line, int column, Object form) {
		this.args = args;
		this.c = c;
		this.form = form;
		...
		if(ctor == null && RT.booleanCast(WARN_ON_REFLECTION.deref()))
			{
			RT.errPrintWriter()
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object frm) {
			...
			for(ISeq s = RT.next(RT.next(form)); s != null; s = s.next())
				args = args.cons(analyze(context == C.EVAL ? context : C.EXPRESSION, s.first()));
			return new NewExpr(c, args, line, column, frm);
		}
	}
...
public static class MetaExpr implements Expr{
	...
	public MetaExpr(Expr expr, Expr meta, Object form){
		this.expr = expr;
		this.meta = meta;
		this.form = form;
	}
...
public static class IfExpr implements Expr, MaybePrimitiveExpr{
	...
	public IfExpr(int line, int column, Expr testExpr, Expr thenExpr, Expr elseExpr, Object form){
...
public static class MapExpr implements Expr{
	...
	public MapExpr(IPersistentVector keyvals, Object form){
	...
	static public Expr parse(C context, IPersistentMap form) {
		...
		Expr ret = new MapExpr(keyvals, form);
...
public static class SetExpr implements Expr{
	...
	public SetExpr(IPersistentVector keys, Object form){
		this.keys = keys;
		this.form = form;
	}
	...
	static public Expr parse(C context, IPersistentSet form) {
		...
		Expr ret = new SetExpr(keys, form);
		if(form instanceof IObj && ((IObj) form).meta() != null)
			return new MetaExpr(ret, MapExpr
					.parse(context == C.EVAL ? context : C.EXPRESSION, ((IObj) form).meta()),
          form);
...
public static class VectorExpr implements Expr{
	...
	public VectorExpr(IPersistentVector args, Object form){
		this.args = args;
		this.form = form;
	}
	...
	static public Expr parse(C context, IPersistentVector form) {
		...
		Expr ret = new VectorExpr(args, form);
		if(form instanceof IObj && ((IObj) form).meta() != null)
			return new MetaExpr(ret, MapExpr
					.parse(context == C.EVAL ? context : C.EXPRESSION, ((IObj) form).meta()),
          form);
...
public static class InstanceOfExpr implements Expr, MaybePrimitiveExpr{
	...
	public InstanceOfExpr(Class c, Expr expr, Object form){
		this.expr = expr;
		this.c = c;
		this.form = form;
	}
...
public static class InvokeExpr implements Expr{
  ...
  // contains metadata
	...
	public InvokeExpr(String source, int line, int column, Symbol tag, Expr fexpr, IPersistentVector args,
      Object form, boolean tailPosition) {
		this.source = source;
		this.fexpr = fexpr;
		this.args = args;
		this.line = line;
		...
		if(fexpr instanceof VarExpr && ((VarExpr)fexpr).var.equals(INSTANCE) && RT.count(form) == 3)
			...
			if(sexpr instanceof ConstantExpr)
				...
				if(val instanceof Class)
					{
					return new InstanceOfExpr((Class) val, analyze(context, RT.third(form)), form);
					}
				}
			}
		...
		if(RT.booleanCast(getCompilerOption(directLinkingKey))
           ...
           && context != C.EVAL)
            ...
            if(false && // don't create StaticInvokeExpr's since we don't handle it in the converter - Ambrose
                ...
                Object vtag = RT.get(RT.meta(v), TAG_KEY);
					...
					if(primc != null)
						return analyze(context,
						               ((IObj)RT.listStar(Symbol.intern(".invokePrim"),
						                                  ((Symbol) form.first()).withMeta(RT.map(TAG_KEY, Symbol.intern(primc))),
...
static public class FnExpr extends ObjExpr{
  ...
  Class jc;
	...
	static Expr parse(C context, ISeq form, String name) {
		ISeq origForm = form;
		FnExpr fn = new FnExpr(tagOf(form), form);
...
//			loader.registerConstants(fn.constantsID, fn.constants.toArray());
		...
		if(fmeta != null)
			fmeta = fmeta.without(LINE_KEY).without(COLUMN_KEY).without(FILE_KEY).without(retkey);
...
public static class FnMethod extends ObjMethod{
	...
	public FnMethod(ObjExpr objx, ObjMethod parent, Object form){
		super(objx, parent, form);
	}
	...
	static FnMethod parse(ObjExpr objx, ISeq form, Object rettag) {
		...
		try
			{
			FnMethod method = new FnMethod(objx, (ObjMethod) METHOD.deref(), form);
...
abstract public static class ObjMethod{
  ...
  public final Object form;
	...
	public ObjMethod(ObjExpr objx, ObjMethod parent, Object form){
		this.parent = parent;
		this.objx = objx;
		this.form = form;
	}
...
public static class LocalBindingExpr implements Expr, MaybePrimitiveExpr, AssignableExpr{
  ...
  // actual symbol with metadata
	...
	public LocalBindingExpr(LocalBinding b, Symbol tag, Symbol sym)
            {
...
public static class BodyExpr implements Expr, MaybePrimitiveExpr{
	...
	public BodyExpr(PersistentVector exprs, Object form){
		this.exprs = exprs;
		this.form = form;
	}
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object frms) {
			...
			return new BodyExpr(exprs, frms);
		}
	}
...
public static class LetFnExpr implements Expr{
  ...
  public final Object form;
	...
	public LetFnExpr(PersistentVector bindingInits, Expr body, Object form){
		...
		public Expr parse(C context, Object frm) {
			...
			try
				...
				for(int i = 0; i < bindings.count(); i += 2)
					{
					Symbol sym = (Symbol) bindings.nth(i);
					Expr init = analyze(C.EXPRESSION, bindings.nth(i + 1), sym.getName());
		...
		public Expr parse(C context, Object frm) {
			...
			while(true){
				...
				try
					...
					for(int i = 0; i < bindings.count(); i += 2)
						...
						if(isLoop)
							...
							if(recurMismatches != null && RT.booleanCast(recurMismatches.nth(i/2)))
								{
								init = new StaticMethodExpr("", 0, 0, null, RT.class, "box", RT.vector(init), null, false);
								if(RT.booleanCast(WARN_ON_REFLECTION.deref()))
									RT.errPrintWriter().println("Auto-boxing loop arg: " + sym);
								}
							else if(maybePrimitiveType(init) == int.class)
								init = new StaticMethodExpr("", 0, 0, null, RT.class, "longCast", RT.vector(init), null, false);
							else if(maybePrimitiveType(init) == float.class)
								init = new StaticMethodExpr("", 0, 0, null, RT.class, "doubleCast", RT.vector(init), null, false);
							}
					...
					if(!moreMismatches)
						return new LetExpr(bindingInits, bodyExpr, isLoop, frm);
					}
				finally
					{
					Var.popThreadBindings();
...
//					HostExpr.emitUnboxArg(objx,gen,primc);
	...
	static class Parser implements IParser{
		public Expr parse(C context, Object frm) {
			...
			return new RecurExpr(loopLocals, args, line, column, source, frm);
		}
	}
...
public static Object preserveTag(ISeq src, Object dst) {
	...
	if (tag != null && dst instanceof IObj) {
		...
		return ((IObj) dst).withMeta((IPersistentMap) RT.assoc(meta, TAG_KEY, tag));
	}
	return dst;
}
    ...
    if(v != null)
      {
        ...
        if(! "clojure/spec/alpha.clj".equals(SOURCE_PATH.deref()) &&
           ! "clojure/spec.clj".equals(SOURCE_PATH.deref())
          )
          {
            final String specns;
            if (null != Namespace.find(Symbol.intern("clojure.spec.alpha"))) {
              specns = "clojure.spec.alpha";
            } else if (null != Namespace.find(Symbol.intern("clojure.spec.alpha"))) {
              specns = "clojure.spec";
            } else {
              specns = null;
            }
          ...
          try
            {
              final Namespace checkns = Namespace.find(Symbol.intern(specns));
              if (checkns != null)
                {
                  final Var check = Var.find(Symbol.intern(specns + "/macroexpand-check"));
                  if ((check != null) && (check.isBound()))
                    check.applyTo(RT.cons(v, RT.list(form.next())));
                }
              Symbol.intern(specns);
            }
...
private static Expr analyzeSeq(C context, ISeq form, String name) {
	...
	if(RT.meta(form) != null && RT.meta(form).containsKey(LINE_KEY))
		line = RT.meta(form).valAt(LINE_KEY);
	if(RT.meta(form) != null && RT.meta(form).containsKey(COLUMN_KEY))
		column = RT.meta(form).valAt(COLUMN_KEY);
	Var.pushThreadBindings(
			...
			if(form instanceof ISeq && Util.equals(RT.first(form), DO))
				...
				return eval(RT.first(s), false);
					...
					&& !(RT.first(form) instanceof Symbol
						&& ((Symbol) RT.first(form)).getName().startsWith("def"))))
				{
...
private static Expr analyzeSymbol(Symbol sym) {
	...
	if(sym.getNamespace() == null) //ns-qualified syms are always Vars
		...
		if(namespaceFor(sym) == null)
			...
			if(c != null)
				{
				if(Reflector.getField(c, sym.getName(), true) != null)
					return new StaticFieldExpr(lineDeref(), columnDeref(), c, sym.getName(), tag, sym);
				throw Util.runtimeException("Unable to find static field: " + sym.getName() + " in " + c);
				}
			}
		}
	//Var v = lookupVar(sym, false);
...
//		return new VarExpr(v, tag);
	...
	if(o instanceof Var)
		...
		if(RT.booleanCast(RT.get(v.meta(),CONST_KEY)))
...
static public Object maybeResolveIn(Namespace n, Symbol sym) {
	...
	else if(sym.getName().indexOf('.') > 0 && !sym.getName().endsWith(".")
			|| sym.getName().charAt(0) == '[')
		{
		return RT.classForName(sym.getName());
		}
	else if(sym.equals(NS))
	...
	if(sym.getNamespace() != null)
		...
		var = NS_VAR;
			...
			if(o == null)
				...
				if(internNew)
					var = currentNS().intern(Symbol.intern(sym.getName()));
					}
			else if(o instanceof Var)
				{
				var = (Var) o;
...
public static Object compile(Reader rdr, String sourcePath, String sourceName) throws IOException{
	...
	try
		...
		ObjExpr objx = new ObjExpr(null, null);
...
static public class NewInstanceExpr extends ObjExpr{
  ...
  public PersistentVector interfaces;
	...
	public NewInstanceExpr(Object tag, Object form){
		super(tag, form);
	}
	...
	static class DeftypeParser implements IParser{
		public Expr parse(C context, final Object frm) {
			...
			ObjExpr ret = build((IPersistentVector)RT.get(opts,implementsKey,PersistentVector.EMPTY),fields,null,tagname, classname,
			             (Symbol) RT.get(opts,TAG_KEY),rform, frm, opts);
			return ret;
		}
	}
	...
	static ObjExpr build(IPersistentVector interfaceSyms, IPersistentVector fieldSyms, Symbol thisSym,
		...
		NewInstanceExpr ret = new NewInstanceExpr(null, frm);
...
//		ret.volatiles = PersistentHashSet.create(RT.seq(RT.get(ret.optionsMap, volatileKey)));
		...
		Map<IPersistentVector,java.lang.reflect.Method> overrideables = mc[0];
		...
		if(this.isDeftype())
			...
			if (this.isDeftype() && this.fields.count() > this.hintedFields.count())
				...
				for(ISeq s = RT.seq(hintedFields); s!=null; s=s.next(), i++)
					{
					String bName = ((Symbol)s.first()).getName();
...
public static class NewInstanceMethod extends ObjMethod{
  ...
  public java.lang.reflect.Method method;
	...
	public NewInstanceMethod(ObjExpr objx, ObjMethod parent, Object form){
		super(objx, parent, form);
	}
	...
	static NewInstanceMethod parse(ObjExpr objx, ISeq form, Symbol thistag,
		...
		NewInstanceMethod method = new NewInstanceMethod(objx, (ObjMethod) METHOD.deref(), form);
		Symbol dotname = (Symbol)RT.first(form);
		Symbol name = (Symbol) Symbol.intern(null,munge(dotname.getName())).withMeta(RT.meta(dotname));
		...
		try
			...
			for(int i = 0; i < parms.count(); i++)
				...
				if(tag != null)
					hinted = true;
				if(p.getNamespace() != null)
					p = Symbol.intern(p.getName());
				Class pclass = tagClass(tag);
				pclasses[i] = pclass;
				psyms[i] = p;
				}
			Map matches = findMethodsWithNameAndArity(name.getName(), parms.count(), overrideables);
			Object mk = msig(name.getName(), pclasses);
			...
			if(matches.size() > 0)
				...
				if(matches.size() > 1)
					...
					if(!hinted)
						throw new IllegalArgumentException("Must hint overloaded method: " + name.getName());
					m = (java.lang.reflect.Method) matches.get(mk);
					if(m == null)
						throw new IllegalArgumentException("Can't find matching overloaded method: " + name.getName());
					if(m.getReturnType() != method.retClass)
						throw new IllegalArgumentException("Mismatched return type: " + name.getName() +
					...
					if(hinted)
						...
						m = (java.lang.reflect.Method) matches.get(mk);
						if(m == null)
							throw new IllegalArgumentException("Can't find matching method: " + name.getName() +
...
//				throw new IllegalArgumentException("Can't override/overload method: " + name.name);
				...
				throw new IllegalArgumentException("Can't define method not in interfaces: " + name.getName());
	...
	static Class primClass(Symbol sym){
		...
		if(sym.getName().equals("int"))
			c = int.class;
		else if(sym.getName().equals("long"))
			c = long.class;
		else if(sym.getName().equals("float"))
			c = float.class;
		else if(sym.getName().equals("double"))
			c = double.class;
		else if(sym.getName().equals("char"))
			c = char.class;
		else if(sym.getName().equals("short"))
			c = short.class;
		else if(sym.getName().equals("byte"))
			c = byte.class;
		else if(sym.getName().equals("boolean"))
			c = boolean.class;
		else if(sym.getName().equals("void"))
			c = void.class;
		return c;
	}
...
public static class CaseExpr implements Expr, MaybePrimitiveExpr{
	...
	public CaseExpr(int line, int column, LocalBindingExpr expr, int shift, int mask, int low, int high, Expr defaultExpr,
	        SortedMap<Integer,Expr> tests,HashMap<Integer,Expr> thens, Keyword switchType, Keyword testType, Set<Integer> skipCheck, Object form){
		this.expr = expr;
		this.shift = shift;
		this.mask = mask;
		this.low = low;
        ...
        if(RT.count(skipCheck) > 0 && RT.booleanCast(WARN_ON_REFLECTION.deref()))
            {
            RT.errPrintWriter()
	...
	static class Parser implements IParser{
		...
		public Expr parse(C context, Object frm) {
			...
			return new CaseExpr(line, column, testexpr, shift, mask, low, high,
			        defaultExpr, tests, thens, switchType, testType, skipCheck, frm);
		}
	}
}
