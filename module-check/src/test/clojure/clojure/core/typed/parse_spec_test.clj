(ns clojure.core.typed.parse-spec-test
  (:require [clojure.spec :as s]
            [clojure.spec.test]
            [clojure.test :refer :all]
            [clojure.tools.analyzer.jvm :as ana.jvm]
            [clojure.core.typed.parse-spec :as c]
            [clojure.core.typed.type-rep :as r]
            [clojure.core.typed.type-ctors :as ct]
            #_[spectrum.check :as check]
            #_[spectrum.flow :as flow])
...
(deftest parse-spec-works
  (testing "returns Spect"
    (is (c/clj-spec->Type (c/parse-spec #'integer?)))
    (is (c/clj-spec->Type (c/parse-spec #'int?)))
    (is (c/clj-spec->Type (c/parse-spec (s/spec integer?))))
    (is (c/clj-spec->Type (c/parse-spec ::even-int)))
    ;; unsupported
    #_(is (c/clj-spec->Type (c/parse-spec #(< % 10))))
    (is (c/clj-spec->Type (c/parse-spec (s/spec ::even-int))))
    (is (c/clj-spec->Type (c/parse-spec (s/or :int integer? :str string?))))
    ;; unsupported
    #_(is (c/clj-spec->Type (c/parse-spec (s/and integer? #(> % 10)))))
    (is (c/clj-spec->Type (c/parse-spec (s/nilable int?))))
    (is (c/clj-spec->Type (c/parse-spec (s/coll-of int?))))
    ;; FIXME what does this spec mean?
    #_(is (c/clj-spec->Type (c/parse-spec '[integer? integer?])))

    (is (= r/-nil (c/clj-spec->Type (c/parse-spec ::s/nil))))

    ;; regex
    (is (= (ct/-name 'clojure.core.typed/Seqable (ct/-name 'clojure.core.typed/Int))
           (c/clj-spec->Type (c/parse-spec (s/* integer?)))))
    (is (= (ct/-name 'clojure.core.typed/Seqable (ct/-name 'clojure.core.typed/Int))
           (c/clj-spec->Type (c/parse-spec (s/+ integer?)))))
    (is (c/clj-spec->Type (c/parse-spec (s/? integer?))))
    (is (c/clj-spec->Type (c/parse-spec (s/alt :x integer? :y keyword?))))
    (is (c/clj-spec->Type (c/parse-spec (s/alt :x (s/* integer?) :y (s/* keyword?)))))
    (is (c/clj-spec->Type (c/parse-spec (s/cat :x integer? :y keyword?))))
    (is (c/clj-spec->Type (c/parse-spec (s/cat :x (s/* integer?) :y keyword?))))
    )
  ...
  (testing "keys"
    (is (c/clj-spec->Type (c/parse-spec (s/keys :req [::even-int]))))
    )
